...
chained mediation analysis based on the prepared dataset. The function
computes regression equations for mediators and the outcome variable,
indirect effects along multi-step mediation paths, total effects,
contrasts between indirect effects, and coefficients in different
conditions.
...
- MP:
  ...
  variable(s) W. Valid entries include: - `"a2"`, `"a3"`, ...:
  moderation on the a paths (W → Mdiff), for mediators beyond M1. -
  `"b2"`, `"b3"`, ...: moderation on the b paths (Mdiff × W → Ydiff). -
  `"b_1_2"`, `"b_2_3"`, ...: moderation on cross-paths from one mediator
  to the next (e.g., M1 → M2). - `"d2"`, `"d3"`, ...: moderation on the
  d paths (Mavg × W → Ydiff). - `"d_1_2"`, `"d_2_3"`, ...: moderation on
  cross-paths from one Mavg to the next Mdiff. - `"cp"`: moderation on
  the direct effect from X to Y (i.e., W → Ydiff).
...
This function is used to construct SEM models for chained mediation
...
- **Mediator regressions**: Defines the sequential regression models for
  each mediator's difference score, incorporating prior mediators as
  predictors.

- **Indirect effects**: Computes the indirect effects along all possible
  multi-step mediation paths using the product of path coefficients.

- **Total indirect effect**: Calculates the sum of all indirect effects
  from the chained mediation paths.
...
This model is suitable for chained mediation designs where mediators
influence each other in a sequential manner, forming multi-step
mediation paths.
